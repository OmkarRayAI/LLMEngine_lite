...
def generate_llm_compiler_prompt(
    ...
    prefix = (
        "Given a user query and meta plan, create a plan to solve it with the utmost parallelizability."
    ...
    prefix += (
        ...
        " - You MUST strictly follow the TEMPORAL CONTEXT provided in the meta plan. The date/time period is critical for financial analysis.\n"
        " - If the meta plan specifies a particular date (either current or historical), all data retrieval actions should be aligned with that timeframe.\n"
        " - Your plan must prioritize the most recent data available within the specified timeframe to provide up-to-date results to the user.\n"
        " - All financial calculations, comparisons, and analyses must be anchored to the temporal context specified in the meta plan.\n"
        " - Follow the meta plan when sequencing your actions.\n"
        "- If the user query is a simple greeting, directly call join.\n\n"
    )
    ...
    if is_replan:
        prefix += (
            ...
            " - Never introduce new actions other than the ones provided.\n"
            " - Ensure that your replan maintains alignment with the temporal context from the meta plan.\n\n"
        )
...
class Planner:
    def __init__(self):
        ...
        self.system_prompt_replan = None
        self.tools = None
    ...
    async def run_llm(
        ...
        if is_replan:
            ...
            system_prompt = self.system_prompt.format(meta_data=meta_data)
            human_prompt = f"Question: {inputs['input']}"
    ...
    async def plan(
        ...
        **kwargs: Any
        ...
        llm= llm,example_prompt= example_prompt,example_prompt_replan= example_prompt_replan,
        tools= tools,stop=stop,
            inputs=inputs, meta_data=meta_data, is_replan=is_replan, callbacks=callbacks
        )
